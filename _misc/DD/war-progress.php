...
// Find the Factions
$wars = 0;
$result0 = mysql_query("SELECT * FROM wars") or die(mysql_error());
...
// Execute Combat
$counter = 0;
$result2 = mysql_query("SELECT * FROM wars") or die(mysql_error());
while ($row2 = mysql_fetch_array( $result2 )){
	$counter++;
	if ($counter == 1){
		$a1damage = intval(rand($a1army/6,$a1army/4));
		$d1damage = intval(rand($d1army/6,$d1army/4));
		if ($a1army <= 0){
			$d1damage = 2*$d1damage;
		}
		else {
		...
		$a2damage = intval(rand($a2army/6,$a2army/4));
		$d2damage = intval(rand($d2army/6,$d2army/4));
		if ($a2army <= 0){
			$d2damage = 2*$d2damage;
		}
		else {
...
mysql_query("UPDATE wars SET casualties='$casualties' WHERE id='1'") or die(mysql_error());
$casualties2 = $casualties2 + $a2damage + $d2damage;
mysql_query("UPDATE wars SET casualties='$casualties2' WHERE id='2'") or die(mysql_error());

// End War
if (isset($attacker1) AND ($a1influence <= 0 OR $d1influence <= 0)){
	mysql_query("DELETE FROM wars WHERE attacker='$attacker1'") or die(mysql_error());
	if ($a1influence <= 0){
		$a1losses = $a1losses + 1;
		$d1wins = $d1wins + 1;
...
if (isset($attacker2) AND ($a2influence <= 0 OR $d2influence <= 0)){
	mysql_query("DELETE FROM wars WHERE attacker='$attacker2'") or die(mysql_error());
	if ($a2influence <= 0){
		$a2losses = $a2losses + 1;
		$d2wins = $d2wins + 1;
